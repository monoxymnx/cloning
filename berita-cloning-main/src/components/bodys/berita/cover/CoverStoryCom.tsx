...
import { coverData } from "../../../../data/berita/coverStory";
import Pagination from "@mui/material/Pagination";

type Props = {};

interface CoverStoryItem {
  img: string;
  title: string;
  detail: string;
  date: string;
...
const CoverComponent = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 13;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems: CoverStoryItem[] = coverData.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="main">
      <Container className="container-love">
        ...
        >
          <Pagination
            count={Math.ceil(coverData.length / itemsPerPage)}
            page={currentPage}
