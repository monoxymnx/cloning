...
import { resturantData } from "../../../../data/berita/resturant";
type Props = {};
interface ResturantItem {
  img: string;
  title: string;
  detail: string;
  date: string;
...
const ResturantComponents = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 13;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems: ResturantItem[] = resturantData.slice(
   indexOfFirstItem,
   indexOfLastItem
 );
  ...
  return (
    <div className="main">
      <Container className="container-love">
        ...
        >
          <Pagination
            count={Math.ceil(resturantData.length / itemsPerPage)}
            page={currentPage}
