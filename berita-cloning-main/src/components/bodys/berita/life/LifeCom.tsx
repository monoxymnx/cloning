...
import { lifeData } from "../../../../data/berita/life";
type Props = {};
interface LifeItem {
  img: string;
  title: string;
...
const LifeComponents = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 13;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
 const currentItems: LifeItem[] = lifeData.slice(
   indexOfFirstItem,
   indexOfLastItem
 );
  ...
  return (
    <div className="main">
      <Container className="container-love">
        ...
        >
          <Pagination
            count={Math.ceil(lifeData.length / itemsPerPage)}
            page={currentPage}
