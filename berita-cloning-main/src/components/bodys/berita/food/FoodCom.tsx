...
import { foodData } from '../../../../data/berita/food';
type Props = {};
interface FoodItem {
  img: string;
  title: string;
...
const FoodComponents = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 13;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems: FoodItem[] = foodData.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="main">
      <Container className="container-love">
        ...
        >
          <Pagination
            count={Math.ceil(foodData.length / itemsPerPage)}
            page={currentPage}
