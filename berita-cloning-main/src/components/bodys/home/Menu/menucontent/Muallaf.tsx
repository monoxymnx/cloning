...
import { MuallafData } from "../data/Muallaf";
import Pagination from "@mui/material/Pagination";
import { Link } from "react-router-dom";

type Props = {};

const Muallaf = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 9;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = MuallafData.slice(indexOfFirstItem, indexOfLastItem);
  ...
  return (
    <Container>
      <Grid container spacing={2} justifyContent="center" alignItems="center">
        <Grid item xs={12}>
          <h2 style={{ textAlign: "start" }}>Muallaf</h2>
        </Grid>
        <Grid
          container
          spacing={2}
        ...
        <Grid item xs={12} style={{ textAlign: "center" }}>
          ...
          >
            <Pagination
              count={Math.ceil(MuallafData.length / itemsPerPage)}
              page={currentPage}
