...
import { BaitulData } from "../data/Baitul";
import Pagination from "@mui/material/Pagination";
import { Link } from "react-router-dom";

type Props = {};

const Baitul = (props: Props) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 9;

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = BaitulData.slice(indexOfFirstItem, indexOfLastItem);
  ...
  return (
    <Container>
      <Grid container spacing={2} justifyContent="center" alignItems="center">
        <Grid item xs={12}>
          <h2 style={{ textAlign: "start" }}>Baitul</h2>
        </Grid>
        <Grid
          container
          spacing={2}
        ...
        <Grid item xs={12} style={{ textAlign: "center" }}>
          ...
          >
            <Pagination
              count={Math.ceil(BaitulData.length / itemsPerPage)}
              page={currentPage}
